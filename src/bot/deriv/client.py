...
class DerivWsClient:
    ...
    def __init__(self, base_url: str, app_id: int):
        ...
        LOG.debug("DerivWsClient initialized: base_url=%s, app_id=%d", self.base_url, self.app_id)
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if attempt < MAX_RETRIES:
                    ...
                    LOG.warning("WebSocket attempt %d failed: %s. Retrying in %ds...", 
                              attempt, e, wait_time)
                    await asyncio.sleep(wait_time)
                else:
                    LOG.exception("WebSocket request failed after %d attempts", MAX_RETRIES)
        
    
        raise Exception(f"WebSocket request failed after {MAX_RETRIES} attempts: {last_error}")
